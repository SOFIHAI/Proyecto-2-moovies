...
function lista() {
  ...
  let tabla =
    "<tr><th>Imagen</th><th>Titulo</th><th>Categoria</th><th>Año</th><th>sinopsis</th><th>Borrar</th><th>Editar</th><th>Favorito</th></tr>";
  ...
  peliculas?.map((pelicula) => {
    if (pelicula.estado == true) {
      ...
      tabla +=
        "<td><button type='button' class='btn btn-danger btnEliminar' id = " +
        "" +
        pelicula.id +
        " ><i class='fa fa-close'></i> Borrar</button></td>";
      tabla +=
        "<td><button type='button' class='btn btn-warning btnModificar' id = " +
        "" +
        pelicula.id +
        " ><i class='fa fa-edit'></i> Modificar</button></td>";
      tabla +=
        "<td><button type='button' class='btn btn-success btnPublicado' id = " +
        "" +
        pelicula.id +
...
function altaModificacion() {
  ...
  if (!isUpdate) {
    ...
    let peliculaExistente = peliculas?.find(
...
function cambiarPublicado(id) {
  let respuesta = confirm(
    "¿Desea cambiar el publicado de la pelicula seleccionada?"
  );
  if (respuesta) {
    debugger;
    let updatePeliculas = peliculas?.map((pelicula) => {
